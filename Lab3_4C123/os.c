...
tcbType tcbs[NUMTHREADS+NUMPERIODIC];
tcbType *RunPt;
int32_t Stacks[NUMTHREADS+NUMPERIODIC][STACKSIZE];
...
void OS_Init(void){
  ...
  // perform any initializations needed
	BSP_PeriodicTask_InitC(&runperiodicevents,1000,6); //1 ms hardware interrupt to decrement sleep;
}
...
uint32_t periodicThreadCnt = 0; // three available BSP_PeriodicTask_Init functions available in BSP
int OS_AddPeriodicEventThread(void(*thread)(void), uint32_t period){
// ****IMPLEMENT THIS****
	if (periodicThreadCnt == 0) {
		periodicThreadCnt++;
		BSP_PeriodicTask_Init(thread,(1000/period),6);
		return 1;
	} else if (periodicThreadCnt == 1) {
		periodicThreadCnt++;
		BSP_PeriodicTask_InitB(thread,(1000/period),6);
		return 1;
	} else{
		return 0;
	}
...
//***IMPLEMENT THIS***
	...
	*semaPt = *semaPt - 1;
	if((*semaPt) < 0) {
		RunPt->blocked = (uint32_t) semaPt; // reason it is blocked
		EnableInterrupts();
